...
from vitac.models.util.transformer import get_1D_position_embeddings
...
class TacConNet:
    ...
    def Encoder_pe(self, encoder_embed_dim):
        self.encoder_embed_dim = encoder_embed_dim
        return nn.Parameter(
            torch.zeros(1, self.tac_num_patches, encoder_embed_dim),
            requires_grad=False,
        )

    def Decoder_pe(self, decoder_embed_dim):
        self.decoder_embed_dim = decoder_embed_dim
        return nn.Parameter(
            torch.zeros(1, self.tac_num_patches, decoder_embed_dim),
            requires_grad=False,
        )
