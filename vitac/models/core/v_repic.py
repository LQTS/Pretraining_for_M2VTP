...
class V_RePic(nn.Module):
    ...
    def encode(self, img: torch.Tensor,) -> torch.Tensor:
        ...
        # Create "dummy" visible mask, concatenate image patches, feed to Transformer
    ...
    def forward_decoder(self, visible_patches: torch.Tensor, restore_idxs: torch.Tensor) -> torch.Tensor:
        ...
        if self.create_mask_pic and not self.mask_pic_created:
            MaskPatches_BlackArea = torch.zeros(
                [8, mask_tokens.shape[-2], (self.patch_size ** 2) * self.in_channels], dtype=torch.float32)
            MaskPatches_WhiteArea = torch.ones(
                [8, projected_patches.shape[-2], (self.patch_size ** 2) * self.in_channels], dtype=torch.float32)
    ...
    def forward(
        ...
        return loss, reconstructions, self.mask_patches
