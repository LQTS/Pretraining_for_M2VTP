...
from vitac.models.util.optimization import get_lr_update
from vitac.models.util.transformer import Block, PatchEmbed, RMSNorm, get_2D_position_embeddings
from vitac.models.util.tactile_encoder import TacConNet
...
class VTT_ReAll(nn.Module):
    ...
    def encode(self, img: torch.Tensor, tac: torch.Tensor) -> torch.Tensor:
        ...
        # Create "dummy" visible mask, concatenate image patches & tactile, feed to Transformer
    ...
    def forward_decoder(
        ...
        if self.create_mask_pic and not self.mask_pic_created:
            MaskPatches_BlackArea = torch.zeros([8, mask_tokens.shape[-2], (self.patch_size**2) * self.in_channels], dtype=torch.float32)
            MaskPatches_WhiteArea = torch.ones([8, projected_patches.shape[-2], (self.patch_size**2) * self.in_channels], dtype=torch.float32)
