...
from vitac.conf import TrackingConfig
...
class Metrics:
    def __init__(
        ...
        elif self.model_arch in {"vtt-repic", "v-repic", "vtt-cond", "vtt-retac", "v-retac", "t-retac"}:
            self.tracker.update({"reconstruction_loss": deque(maxlen=self.window)})

        elif self.model_arch in {"vtt-reall", "v-reall"}:
            self.tracker.update(
                {
    ...
    def push(self, epoch: int) -> str:
        ...
        elif self.model_arch in {"vtt-cond", "vtt-condv2"}:
            self.log(
                self.global_step,
                metrics={
                    "Pretrain/Step": self.global_step,
        ...
        elif self.model_arch in {"vtt-repic", "v-repic"}:
            self.log(
                self.global_step,
                metrics={
                    "Pretrain/Step": self.global_step,
